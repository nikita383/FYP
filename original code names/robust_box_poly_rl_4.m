function [w, risk, ret, sharpe, Lambda,mat] = robust_box_poly_rl_4(n, m,p,f,asset_ret,omega,alpha, Gamma)
...
[(1-eta)*nu, -y0'*F*inv(M), -Delta*w'*e_n, y0';...
    -inv(M)'*F'*y0, (lambda(1)*e_m*e_m'+diag(lambda(2:end))), zeros(m,1), zeros(m,m);...
    -Delta*e_n'*w, zeros(1,m), (2-Lambda), zeros(1,m);...
    y0, zeros(m,m), zeros(m,1), inv(F)]>=0;
...
cvx_end
mat = [(1-eta)*nu, -y0'*F*inv(M), -Delta*w'*e_n, y0';...
    -inv(M)'*F'*y0, (lambda(1)*e_m*e_m'+diag(lambda(2:end))), zeros(m,1), zeros(m,m);...
    -Delta*e_n'*w, zeros(1,m), (2-Lambda), zeros(1,m);...
    y0, zeros(m,m), zeros(m,1), inv(F)];
risk = w'*(V'*F*V)*w;
ret=(mu0-gam)'*w;
sharpe=ret/risk;
end