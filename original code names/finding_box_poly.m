...
y0 = V0*w;
tau >= 0;
[inv_tau_nu zeros(1,m) y0' Delta_hat*w'*e_n; zeros(m,1) e_m*e_m' inv(M)' zeros(m,1); y0 inv(M) tau*F zeros(m,1); Delta_hat*e_n'*w zeros(1,m) zeros(1,m) 1]>=0;

for i = 1:m,
    inv_tau_i_tau(i) >= 0;
    e_i = eye(m);
    e_i = e_i(:,i);
    [inv_tau_nu zeros(1,m) y0' Delta*w'*e_n; zeros(m,1) inv_tau_i_tau(i)*e_i*e_i' inv(M)' zeros(m,1); y0 inv(M) tau*F zeros(m,1); Delta*e_n'*w zeros(1,m) zeros(1,m) inv_tau_i_tau(i)]>=0;
end

[-nu tau; inv_tau_nu -1]>=0;
