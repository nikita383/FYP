function [w, risk, ret, sharpe] = robust_box_poly_rl(n, m,p,f,asset_ret,omega,alpha, Gamma)
...
variable delta(1);
variable tau(1);
variable inv_tau_nu(1);
variable inv_tau_i_tau(m,1);
variable nu(1);
...
y0 = V0*w;
tau >= 0;
[(1-eta)*inv_tau_nu zeros(1,m) y0' Delta_hat*w'*e_n; zeros(m,1) e_m*e_m' inv(M)' zeros(m,1); y0 inv(M) tau*F zeros(m,1); Delta_hat*e_n'*w zeros(1,m) zeros(1,m) 1]>=0;

for i = 1:m,
    inv_tau_i_tau(i) >=0;
    e_i = eye(m);
    e_i = e_i(:,i);
    [(1-eta)*inv_tau_nu zeros(1,m) y0' Delta*w'*e_n; zeros(m,1) inv_tau_i_tau(i)*e_i*e_i' inv(M)' zeros(m,1); y0 inv(M) tau*F zeros(m,1); Delta*e_n'*w zeros(1,m) zeros(1,m) inv_tau_i_tau(i)]>=0;
end

[(1+nu) nu 1 (inv_tau_nu + tau); nu (1+nu) 0 0; 1 0 (1+nu) 0; (inv_tau_nu + tau) 0 0 (1+nu)]>=0; %% works most feasibly
% [nu 0.5*(tau +inv_tau_nu); 0.5*(tau+inv_tau_nu) 1]>=0; %% barely feasible and results suck
