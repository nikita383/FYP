function [w, risk, ret, sharpe] = robust_poly_rl(n, m,p,f,asset_ret,omega,alpha, Gamma)
...
y0 = V0*w;
Lambda >= 0;
[(1-eta)*inv_Lambda_nu zeros(1,m) y0' Delta_hat*w'*e_n;...
    zeros(m,1) (lambda_hat(1)*e_m*e_m'+diag(lambda_hat(2:end))) inv(M)' zeros(m,1);...
    y0 inv(M) Lambda*inv(F) zeros(m,1); Delta_hat*e_n'*w zeros(1,m) zeros(1,m) 1]>=0;
